        ...
        <div class="col-md-12">
            <form action="{{ route('filters.store') }}" method="POST">
                ...
                <div class="tab-content p-4 border bg-white border-top-0" id="custom-tabs-one-tabContent">
                    ...
                    <div class="tab-pane fade @if($k === 0) show active @endif" id="tab-{{$k}}" role="tabpanel" aria-labelledby="custom-tabs-one-home-tab">
                        <div class="form-group">
                            ...
                            @php 
                                $value = empty($filter) ?  '': $filter->getTranslation('title', $locale);
                            @endphp
                ...
                <div class="form-group">
                    ...
                    <textarea name="options" class="form-control">{{ $filter->options ?? '' }}</textarea>
                </div>

                <div class="form-group">
                    <label for="active">Active:</label>
                    <input type="checkbox" name="active" {{ $filter->active ?? false ? 'checked' : '' }}>
                </div>
